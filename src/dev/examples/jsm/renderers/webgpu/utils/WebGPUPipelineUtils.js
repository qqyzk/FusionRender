...
import { ShadersUtils } from './shaders.js';
...
class WebGPUPipelineUtils {
	...
	constructor( backend ) {
		...
		this.shaderUtils = new ShadersUtils();

	}

	createRenderPipelineBySignature(objectSignature){

		const backend = this.backend;
		const device = backend.device;
		const vertexBuffers = [];

		for ( const attribute of objectSignature.attributes ) {
			vertexBuffers.push( {
		...
		let pipelineLayout;
		if(objectSignature.isInstanceMesh){
			const layout = device.createBindGroupLayout({
				...
				{
					binding: 2,  // 第三个绑定点
					visibility: GPUShaderStage.VERTEX,  // 这里可以根据需要更新可见性
					buffer: {
					type: "read-only-storage",  // 这里可以根据需要更新绑定类型
					hasDynamicOffset: false,
					},
				},
				{
		...
		return device.createRenderPipeline( {
			vertex:  {
				module: device.createShaderModule( { code: objectSignature.vertexShader, label: 'vertex' } ),
				entryPoint: 'main',
				buffers: vertexBuffers  
			},
			fragment: {
				module: device.createShaderModule( { code: objectSignature.fragmentShader, label: 'fragment' } ),
				entryPoint: 'main',
			 	targets: [ {
					format: objectSignature.colorFormat,
					blend: {
						alpha: {
							srcFactor: objectSignature.alphaBlendSrcFactor,
							dstFactor: objectSignature.alphaBlendDstFactor,
							operation: objectSignature.alphaBlendOperation
						},
						color: {
							srcFactor: objectSignature.colorBlendSrcFactor,
							dstFactor: objectSignature.colorBlendDstFactor,
							operation: objectSignature.colorBlendOperation
						}
					},
					writeMask: objectSignature.colorWriteMask
				}]
			},
			primitive: {
				topology: objectSignature.topology,
				stripIndexFormat: objectSignature.stripIndexFormat,
				frontFace: objectSignature.frontFace,
				cullMode: objectSignature.cullMode
			},
			depthStencil: {
				format: objectSignature.depthStencilFormat,
				depthWriteEnabled: objectSignature.depthWriteEnabled,
				depthCompare: objectSignature.depthCompare,
				stencilFront:  {
					compare: objectSignature.stencilCompare,
					failOp: objectSignature.stencilFailOp,
					depthFailOp: objectSignature.stencilDepthFailOp,
					passOp: objectSignature.stencilPassOp
				},
				stencilBack: {}, 
				stencilReadMask: objectSignature.stencilReadMask,
				stencilWriteMask: objectSignature.stencilWriteMask
			},
			multisample: {
				count: objectSignature.sampleCount
			},
	...
	createRenderPipelineBySignature2(objectSignature,limit,useUniform,useSplit,lightsList){
		const backend = this.backend;
		const device = backend.device;
		const vertexBuffers = [];
		...
		const shaders = this.shaderUtils.getShadersFixed(objectSignature.shaderType,limit,useUniform,useSplit,lightsList);
		// const shaders = this.shaderUtils.getShadersFixedTry();
		return device.createRenderPipeline( {
			vertex:  {
				module: device.createShaderModule( { code: shaders.vertexShader, label: 'vertex' } ),
				entryPoint: 'main',
				buffers: vertexBuffers  
			},
			fragment: {
				module: device.createShaderModule( { code: shaders.fragmentShader, label: 'fragment' } ),
				entryPoint: 'main',
			 	targets: [ {
					format: objectSignature.colorFormat,
					blend: {
	...
	mycreateRenderPipeline( vertexModule, fragmentModule,renderObject,attributes,context) {
		...
		const shaderAttributes = backend.attributeUtils.mycreateShaderAttributes( attributes );
	...
	_getStepMode( geometryAttribute ) {
		return ( geometryAttribute !== undefined && geometryAttribute.isInstancedBufferAttribute ) ? GPUInputStepMode.Instance : GPUInputStepMode.Vertex;
	}

	_getNeedBlend( material ) {
		return material.transparent === true && material.blending !== NoBlending;
	}
